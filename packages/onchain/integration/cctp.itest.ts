...
// Burns 0.05 USDC on Base Sepolia (domain 6) → Iris sandbox attests → mints native
// USDC on Arc (domain 26). Pre-flight verified the CCTP V2 contracts ARE live on
// Arc (MessageTransmitter.localDomain() == 26, 2026-06-13). mintRecipient is OUR
// OWN wallet, so the bridged USDC returns to us — the only loss-case is a burn that
// never mints (bounded to the tiny amount).
//
...
// Reads .env (bun auto-loads): SERVER_WALLET_PRIVATE_KEY, BASE_SEPOLIA_RPC_URL,
...
suite("live CCTP V2 — Base Sepolia → Arc (real burn, real mint)", () => {
  test(
    "bridge 1 USDC: burn on Base Sepolia → attest → mint on Arc",
    async () => {
      ...
      const srcRpc = process.env.BASE_SEPOLIA_RPC_URL;
      const dstRpc = process.env.ARC_RPC_URL;
      const src = CCTP_SOURCE_CHAIN; // baseSepolia
      ...
      console.log(
        `\n[CCTP LIVE] burn (Base Sepolia): ${burnTxHash}\n[CCTP LIVE] mint (Arc):          ${mintTxHash}\n`
      );
