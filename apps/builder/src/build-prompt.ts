...
- OPENING — open the way the app PLAYS; do NOT paste a bright banner. Choose: (a) a **game/arcade** drops straight onto the HUD (a big \`.tj-stat\` + the action) — no header; (b) a **quiz / personality / roast / generator** opens with \`.tj-hero\` — a GLOWING TITLE on the dark stage (gradient text + soft glow, NOT a bright slab): \`<div className="tj-hero"><h1 className="tj-title">…</h1><p className="tj-sub">…</p></div>\`; (c) a **poll / wall** shows a compact \`.tj-header\` then jumps to the ballot/feed; (d) a **travel / photo / visual** jam uses \`.tj-hero-art\` (a baked \`<img src="/hero.png">\` as the first child — its edges fade into the dark — then \`.tj-title\`). Pick ONE that fits; never repeat the jam name as a giant slab (the host bar already shows it).
...
- Match the opening to the app (see OPENING above) — a title screen, an art hero, a compact header, or straight into the HUD. Need a one-off style? Add a NEW class in globals.css or an inline style for layout. The PAGE itself stays the DARK stage with LIGHT text: NEVER set a LIGHT/white page/body/\`.tj-app\` background and never put light text on a light fill. Never edit theme.css or its \`:root\` tokens. (A green build with an unreadable or off-theme UI is a FAILURE and will be rejected.)
...
A VETTED contract is ALREADY seeded + filled at \`contracts/src/Game.sol\` (a chance / tic-tac-toe / collectible template chosen from the spec), and a working starter \`app/page.tsx\` is in place. **DO NOT rewrite the Solidity** — it's correct, compile-tested, and operator-gated; the builder compiles + deploys it to Base and wires sdk.onchain to its address. Your job is ONLY the game UI.
